...
export class MemSearchTool extends StructuredTool {
  ...
  async _call(args: z.infer<this['schema']>) {
    const search_query = args.search_query ?? args.query
    const max_results = args.max_results ?? args.k ?? 5
    const threshold = typeof args.threshold === 'number' ? args.threshold : 0.5
    try {
      if (!search_query) throw new Error("Missing 'search_query' (or 'query') parameter")
